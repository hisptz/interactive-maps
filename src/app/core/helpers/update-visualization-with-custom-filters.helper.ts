...
export function updateVisualizationWithCustomFilters(
  ...
  const metadataIdentifiers = layers
    .map(visualizationLayer => _.find(visualizationLayer.filters, ['name', 'dx']))
  ...
  filterItems.forEach(filterItem => {
    if (dimensionType === 'pe') {
      ...
      newFilterItems.push({
        ...
        displayName: filterItem.name,
        dimensionItemType:
          filterItem.id.indexOf('LEVEL') !== -1
            ? 'LEVEL'
            : filterItem.id.indexOf('OU_GROUP') !== -1
              ? 'GROUP'
              : 'ORGANISATION_UNIT'
      });
