...
export class VisualizationEffects {
  ...
  @Effect()
  LoadMapFavourites$ = this.actions$.pipe(
    ofType(VisualizationActionTypes.LOAD_MAP_FROM_FAV),
    map((action: CreateVisualizationAction) => action.favouriteID),
    flatMap(favouriteID => {
      return this.favouriteService.getMapFromFav(favouriteID).pipe(
  ...
  @Effect()
  TransformFavourite$ = this.actions$.pipe(
    ofType(VisualizationActionTypes.TRANSFORM_FAVOURITE),
    map((action: TransformFavouriteAction) => {
      ...
      const visualization = updateVisualizationWithSettings(
  ...
  LoadLegendSets$ = this.actions$.pipe(
    ofType<LoadLegendSet>(VisualizationActionTypes.LOAD_LEGENDSET),
    flatMap((action: LoadLegendSet) => {
      ...
      const legendSetPromise$ = visualizationLayers.map(({ settings }) =>
        this.getLegendsPromise(settings.legendSet)
      );
  ...
  loadAnalytics$ = this.actions$.pipe(
    ofType<LoadVisualizationAnalyticsAction>(
      VisualizationActionTypes.LOAD_ANALYTICS
    ),
            ...
            filter => {
              return filter.name === "dx"
                ? {
                    ...
                    value: _.map(
                      normalDxItems,
                      item => item.dimensionItem || item.id
                    ).join(";")
                  }
                : filter;
            }
      ...
      return forkJoin(analyticsPromises).pipe(
        ...
        map(
          (visualizationObjectResult: Visualization) =>
            new LoadVisualizationSuccessAction(visualizationObjectResult)
        ),
        catchError(error => of(new LoadVisualizationFailAction(error)))
      );
    })
  ...
  private getLegendsPromise(legendSet: any): Observable<any> {
    ...
    return needLegends
      ? this.httpClient.get(
          `legendSets/${legendSet.id}.json?fields=${fields.join(",")}`
        )
      : of(null);
  }
