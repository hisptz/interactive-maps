...
export class MapComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (vizObject) {
      ...
      this.visualizationObject$ = this.store.select(
        fromStore.getCurrentVisualizationObject(id)
      );
    }
  }
  ...
  transhformFavourites(favourite) {
    ...
    if (visObject["layers"].length) {
      this.store.dispatch(
        new fromStore.CreateVisualizationObject(this.visualizationObject)
      );
  ...
  toggleLegendContainerView() {
    this.store.dispatch(
      new fromStore.ToggleOpenVisualizationLegend(this.componentId)
    );
  }
}
